...
<body>
  ...
  {% if has_phages %}
  <h2>Phage Contigs ({{ phage_rows | length }})</h2>
  <table id="phage-table" class="display" style="width:100%">
    <thead>
      <tr>
        <th>Contig</th>
        <th>Length (bp)</th>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>{{ row.contig_length }}</td>
        <td>{{ "%.3f" | format(row.confidence) }}</td>
        <td class="tax-label" title="{{ row.taxonomy_lineage }}">{{ row.taxonomy }}</td>
      </tr>
      {% endfor %}
    </tbody>
  </table>
  ...
  {% if has_chromosomes %}
  <h2>Chromosome Contigs ({{ chromosome_rows | length }})</h2>
  <table id="chromosome-table" class="display" style="width:100%">
    <thead>
      <tr>
        <th>Contig</th>
        <th>Length (bp)</th>
  ...
  {% if has_others %}
  <h2>Other Contigs — Archaea / Unclassified ({{ other_rows | length }})</h2>
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      {% if has_phages %}
      $('#phage-table').DataTable({order: [[1, 'desc']], pageLength: 25});
      {% endif %}
      {% if has_chromosomes %}
      $('#chromosome-table').DataTable({order: [[1, 'desc']], pageLength: 25});
      {% endif %}
      {% if has_others %}
      $('#other-table').DataTable({order: [[1, 'desc']], pageLength: 25});
      {% endif %}
    });
...
@dataclass
class NonPlasmidRow:
    """One row in the chromosome / phage / archaea / unclassified detail tables."""

    contig_id: str
    contig_length: int
    label: str  # chromosome | phage | archaea | unclassified
    confidence: float
    taxonomy: str = "—"  # LCA display string from DIAMOND + GTDB
    taxonomy_lineage: str = "—"  # Full GTDB lineage (tooltip / download)
...
def build_report_data(pipeline_result, input_file: str = "") -> dict:
    ...
    # ── Non-plasmid contig rows (chromosome / phage / archaea / unclassified) ──
    ...
    for npr in non_plasmid_results:
        ...
        if npr.prediction.label == "phage":
            phage_rows.append(row)
        elif npr.prediction.label == "chromosome":
