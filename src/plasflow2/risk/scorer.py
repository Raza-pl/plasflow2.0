...
Scoring formula (capped at 10):
    ...
    Pathogen host:   ESKAPE/ESKAPEE genus or Enterobacteriaceae=2
                     WHO critical/high priority (non-ESKAPE)=1

The pathogen-host factor implements the MetaCompare 2.0 Human Health Resistome
Risk framework (Rumi et al. 2024) and WHO 2024 Bacterial Priority Pathogens List:
ARGs carried by (or taxonomically assigned to) ESKAPE pathogens represent an
acute clinical threat regardless of mobility class.

ESKAPE pathogens (ESKAPEE variant):
    Enterococcus faecium, Staphylococcus aureus, Klebsiella pneumoniae,
    Acinetobacter baumannii, Pseudomonas aeruginosa, Enterobacter spp.,
    Escherichia coli

Week 3 — Day 19 implementation target; Day 29 ESKAPE extension.
"""
...
def _detect_pathogen_host(taxonomy: TaxResult | None) -> tuple[bool, str, int]:
    ...
    Returns:
        Tuple (is_pathogen_host, matched_name, score_bonus):
            ...
            - ``matched_name``: The matched genus (or family) name, empty string
              if no match.
            - ``score_bonus``: 2 for ESKAPE core, 1 for WHO priority non-ESKAPE,
              0 if no match.
    """
    if taxonomy is None:
        return False, "", 0
    ...
    # ESKAPE core genus match (highest priority) → +2
    if genus in ESKAPE_GENERA:
        return True, genus, 2

    # Enterobacteriaceae family match (incomplete taxonomy → still ESKAPE level) → +2
    if family in ESKAPE_FAMILIES:
        return True, family, 2

    # WHO priority genus (non-ESKAPE) → +1
...
def score_plasmid(
    ...
    if is_pathogen and host_score == 2:
        evidence.append(f"ESKAPE pathogen host: {matched_genus} (+2)")
    elif is_pathogen and host_score == 1:
        evidence.append(f"WHO priority pathogen host: {matched_genus} (+1)")
